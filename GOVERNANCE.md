...
| ------ | -------- | ----------------------------------- | ----------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Now    | P0       | Active portfolio                    | Restore deterministic repository gates and establish a trustworthy baseline without changing paused or archived repositories. | No red default branches, clean-checkout canonical check/build results, Compose validation, and explicit red/amber/green outcome per Active repository. |
| Now    | P0       | Backend monoliths and web templates | Replace prose parity with shared black-box conformance.                                                                       | One versioned suite passes against all six monoliths; one web smoke verifies shell and health behavior across all four web templates.                  |
| Now    | P1       | Renovate                            | Reduce the dependency queue and make missing-PR reasons observable.                                                           | Lifecycle-aligned installation scope, explicit limits, major approval, one canary per lane, and archived automation removed.                           |
| Next   | P1       | Local setup and runtime artifacts   | Make recursive checkout, canonical local gates, and production start artifacts reproducible.                                  | HTTPS-compatible submodules, clean-room setup smoke, local/CI gate agreement, and production-start smoke after every build.                            |
