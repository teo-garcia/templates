...
| Titles       | Page titles use `{shortName}                                                                          | {page}`. |
...
| ------ | -------- | ----------------------------------- | ----------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Now    | P0       | `tanstack-template-fullstack`       | Align the declared production start command and container runtime with the artifact the canonical build actually emits.       | Frozen install, canonical check, production build, local production-start smoke, and container entrypoint smoke all pass from a clean checkout.       |
| Now    | P0       | `expo-template-mobile`              | Make dependency versions, public environment access, export/native flows, and test shutdown agree with the supported Expo contract. | Expo dependency validation, frozen install, canonical check, non-interactive test exit, web export, and environment smoke all pass from a clean checkout. |
| Now    | P1       | Renovate                            | Contain automation to lifecycle policy, publish one preset, validate one canary per lane, then drain the dependency queue without flooding maintainers. | Paused repositories expose security fixes for human critical-only triage; planned, incubating, and archived automation is disabled; limits and timezone are explicit; majors require approval; canaries are green; every suppressed update is explained by the dashboard or logs. |
| Next   | P0       | Backend monoliths and web templates | Replace prose parity with shared black-box conformance.                                                                       | One versioned suite passes against all six monoliths; one web smoke verifies shell and health behavior across all four web templates.                  |
| Next   | P0       | Active portfolio                    | Evaluate every Active repository from a clean checkout and repair deterministic local, CI, Compose, or production-start failures one at a time. | No red default branches; canonical check/build, Compose validation, and production-start results are reproducible for every Active repository.         |
| Next   | P1       | Test depth                          | Strengthen weak or misleading Active suites without inflating coverage.                                                       | Domain and HTTP contract coverage for Nest and Spring, real persistence integration for Gin, and clean test-process exit for Expo.                     |
