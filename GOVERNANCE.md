...
| `expo-template-mobile` / `diff@7.0.0` | Accept GHSA-73rr-hh4g-fpgx through `2026-10-05`. | Latest `eas-cli@23.2.0` pins this developer-only dependency and calls `diffLines`; the advisory affects `parsePatch` and `applyPatch`. Review on any EAS update or by the deadline. |
| `expo-template-mobile` / `uuid@8.3.2`, `11.1.1` | Accept GHSA-w5hq-g745-h8pq and its duplicate through `2026-10-05`. | Latest EAS and its Rudder SDK use v4 generation plus validation helpers; the vulnerable v3/v5/v6 caller-buffer path is unused and the CLI is not bundled. Review on any EAS update or by the deadline. |
| `expo-template-mobile` / `ts-deepmerge@6.2.0` | Accept GHSA-87mf-gv2c-c62c through `2026-10-05`. | Latest EAS pins it only in the developer-only `eas new` project generator; it is absent from application runtime. Review on any EAS update or by the deadline. |
...
| ------ | -------- | --------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Now    | P1       | Expo native delivery                                | Verify the pinned EAS profiles against an authenticated Expo project without storing credentials in the template.                               | `eas config` resolves for Android and iOS and one credentialed preview build completes; account credentials and store submission remain consumer-owned.                               |
